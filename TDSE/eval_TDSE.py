...
for num_stages in [1, 2, 4, 8]:
    ...
    t_cortes = [0, 1, 2]
    idx_cortes = [np.argmin(np.abs(t_full - t_val)) for t_val in t_cortes]
    for idx, t_val in zip(idx_cortes, t_cortes):
        plt.figure()
        plt.plot(x_np, psi_mod2_full[:, idx], label=f'PINN t={t_val}')
        plt.plot(x_np, psi_analitica_full[:, idx], '--', label=f'Analítica t={t_val}')
        plt.xlabel('x')
        plt.ylabel('|ψ(x, t)|²')
        plt.title(f'Comparación de la densidad para t={t_val} ({num_stages} etapas)')
        plt.legend()
        plt.tight_layout()
        plt.savefig(f"corte_temporal_t{t_val}_{num_stages}etapas.png")
        plt.close()
