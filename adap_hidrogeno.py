...
def physics_loss(model, r):
    ...
    mask_physics = r.squeeze() <= r_cutoff
    mask_asymptotic = r.squeeze() > r_cutoff
...
def train_model_adaptativo():
    ...
    for epoch in range(max_epochs):
        ...
        if (epoch + 1) % refine_every == 0 and r_train.shape[0] < max_total_points:
            r_dense = torch.linspace(r_min, r_cutoff, 500, device=device).view(-1, 1)
            r_dense.requires_grad_()
            u_dense = model(r_dense)
            du_dr = torch.autograd.grad(u_dense, r_dense, grad_outputs=torch.ones_like(u_dense), create_graph=True)[0]
            d2u_dr2 = torch.autograd.grad(du_dr, r_dense, grad_outputs=torch.ones_like(du_dr), create_graph=True)[0]
            V_dense = -1.0 / torch.sqrt(r_dense**2 + epsilon**2)
            H_u = -0.5 * d2u_dr2 + V_dense * u_dense
            residuo = torch.abs(H_u - model.E * u_dense).detach().cpu().numpy().flatten()
            idx_top = np.argpartition(-residuo, num_points_add)[:num_points_add]
            r_new = r_dense[idx_top].detach()
            r_train = torch.cat([r_train, r_new], dim=0)
            if r_train.shape[0] > max_total_points:
                r_train = r_train[:max_total_points]
            print(f"Refinamiento adaptativo: añadidos {num_points_add} puntos en epoch {epoch+1} (total: {r_train.shape[0]})")
        elif r_train.shape[0] == max_total_points and not lr_switched:
            # Cambia el learning rate solo una vez cuando se alcanza el máximo de puntos
            for param_group in optimizer.param_groups:
                param_group['lr'] = 1e-4
            lr_switched = True
            print(f"Learning rate cambiado a 1e-4 en epoch {epoch+1} (malla completa alcanzada)")
        ...
        if epoch > window:
            ...
            std = np.std(E_window)
            # Pendiente por regresión lineal
            x = np.arange(window)
    ...
    torch.save(model.state_dict(), "modelo_FINAL_hidrogeno.pth")
    print(f" Modelo guardado: modelo_FINAL_hidrogeno.pth")
    ...
    np.save("r_vals_hidrogeno_FINAL.npy", r_np)
    np.save("u_pred_hidrogeno_FINAL.npy", u_pred_norm.squeeze())
    np.save("loss_history_hidrogeno_FINAL.npy", np.array(loss_history))
    np.save("E_history_hidrogeno_FINAL.npy", np.array(E_history))
    np.save("num_points_history_hidrogeno_FINAL.npy", np.array(num_points_history))
    np.save("resultados_entrenamiento_hidrogeno_FINAL.npy", resultados)
...
if __name__ == "__main__":
    print(" Entrenando PINN para el átomo de hidrógeno 1d...")
    train_model_adaptativo()