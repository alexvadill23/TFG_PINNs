...
if __name__ == "__main__":
    ...
    psi_pred_adap, psi_ana_adap, x_plot, y_plot = eval_model_2d(
        model_path=f"modelo_n{n_x}_{n_y}_2d_adap.pth",
        x_path=f"x_vals_n{n_x}_{n_y}_2d_adap.npy",
        n_x=n_x, n_y=n_y
    )
    energia_adap = np.load(f"energia_history_n{n_x}_{n_y}_2d_adap.npy")
    save_energy_fig(energia_adap, n_x+n_y+1.0, f"evolucion_energia_adap_2d_n{n_x}_{n_y}.png")
    save_comparativa_2d_fig(
        psi_pred_adap, psi_ana_adap, x_plot, y_plot,
        f"funcion_onda_adaptativa_2d_n{n_x}_{n_y}.png", tipo="onda",
        titulo=f"Función de onda 2D: PINN vs Analítica (Adaptativa) n_x={n_x}, n_y={n_y}"
    )
    save_comparativa_2d_fig(
        psi_pred_adap, psi_ana_adap, x_plot, y_plot,
        f"densidad_probabilidad_adaptativa_2d_n{n_x}_{n_y}.png", tipo="prob",
        titulo=f"Densidad de probabilidad 2D: PINN vs Analítica (Adaptativa) n_x={n_x}, n_y={n_y}"
    )
    save_corte_y0_fig(psi_pred_adap, psi_ana_adap, x_plot, y_plot,
                      f"corte_y0_malla_adaptativa_2d_n{n_x}_{n_y}.png", f"Corte y=0 malla adaptativa n_x={n_x}, n_y={n_y}")
    save_corte_x0_fig(psi_pred_adap, psi_ana_adap, x_plot, y_plot,
                      f"corte_x0_malla_adaptativa_2d_n{n_x}_{n_y}.png", f"Corte x=0 malla adaptativa n_x={n_x}, n_y={n_y}")
    ...
    save_energy_fig(energia_fija, n_x+n_y+1.0, f"evolucion_energia_fija_2d_n{n_x}_{n_y}.png")
    save_comparativa_2d_fig(
        psi_pred_fija, psi_ana_fija, x_plot, y_plot,
        f"funcion_onda_fija_2d_n{n_x}_{n_y}.png", tipo="onda",
        titulo=f"Función de onda 2D: PINN vs Analítica (Fija) n_x={n_x}, n_y={n_y}"
    )
    save_comparativa_2d_fig(
        psi_pred_fija, psi_ana_fija, x_plot, y_plot,
        f"densidad_probabilidad_fija_2d_n{n_x}_{n_y}.png", tipo="prob",
        titulo=f"Densidad de probabilidad 2D: PINN vs Analítica (Fija) n_x={n_x}, n_y={n_y}"
    )
    save_corte_y0_fig(psi_pred_fija, psi_ana_fija, x_plot, y_plot,
                      f"corte_y0_malla_fija_2d_n{n_x}_{n_y}.png", f"Corte y=0 malla fija n_x={n_x}, n_y={n_y}")
    save_corte_x0_fig(psi_pred_fija, psi_ana_fija, x_plot, y_plot,
                      f"corte_x0_malla_fija_2d_n{n_x}_{n_y}.png", f"Corte x=0 malla fija n_x={n_x}, n_y={n_y}")

    # --- MÉTRICAS Y PRINT FINAL ---
    puntos_fija = len(np.load(f"x_vals_n{n_x}_{n_y}_2d_fija.npy"))
    puntos_adap = len(np.load(f"x_vals_n{n_x}_{n_y}_2d_adap.npy"))
    ...
    try:
        info_adap = np.load(f"training_info_n{n_x}_{n_y}_2d_adap.npy", allow_pickle=True).item()
    ...
    mse1_fija = np.mean((psi_pred_fija - psi_ana_fija)**2)
    mse2_fija = np.mean((-psi_pred_fija - psi_ana_fija)**2)
    if mse1_fija < mse2_fija:
        psi_pred_fija_corr = psi_pred_fija
        mse_fija = mse1_fija
    else:
        psi_pred_fija_corr = -psi_pred_fija
        mse_fija = mse2_fija

    mse1_adap = np.mean((psi_pred_adap - psi_ana_adap)**2)
    mse2_adap = np.mean((-psi_pred_adap - psi_ana_adap)**2)
    if mse1_adap < mse2_adap:
        psi_pred_adap_corr = psi_pred_adap
        mse_adap = mse1_adap
    else:
        psi_pred_adap_corr = -psi_pred_adap
        mse_adap = mse2_adap
