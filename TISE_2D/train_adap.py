...
def train_adaptativo_2d(
    n_x, n_y,
    plot_result=True,
):
    print(f"\n=== ENTRENAR ESTADO n=({n_x},{n_y}) 2D ===")
    ...
    x = torch.linspace(-L, L, int(np.sqrt(num_points_init)), device=device)
    y = torch.linspace(-L, L, int(np.sqrt(num_points_init)), device=device)
    ...
    for epoch in range(30000):
        ...
        # Simetría (opcional, solo si quieres forzar paridad)
        ...
        for kx in range(n_x+1):
            for ky in range(n_y+1):
                if (kx, ky) == (n_x, n_y):
                    continue
                fname = f"psi_pred_n{kx}_{ky}_2d_adaptativo.npy"
        ...
        if (epoch+1) % refine_every == 0:
            ...
            idx_mayor_residuo = np.argpartition(-residuo, num_points_add)[:num_points_add]
            x_new = x_dense_grid[idx_mayor_residuo].detach()
            x_train = torch.cat([x_train, x_new], dim=0)
            print(f"Refinamiento época {epoch+1}: +{num_points_add} puntos añadidos, total: {x_train.shape[0]}")
    ...
    print(f"Estado final: E={current_energy:.6f}, Puntos={x_train.shape[0]}")
    print(f"Total de épocas: {epoch + 1}, loss final: {loss_history[-1]:.4f}")

    # Guardar resultados
    torch.save(model.state_dict(), f"modelo_n{n_x}_{n_y}_2d_adaptativo.pth")
    model.eval()
    with torch.no_grad():
    ...
    np.save(f"x_vals_n{n_x}_{n_y}_2d_adaptativo.npy", x_np)
    np.save(f"psi_pred_n{n_x}_{n_y}_2d_adaptativo.npy", psi_pred_norm.squeeze())
    np.save(f"energia_history_n{n_x}_{n_y}_2d_adaptativo.npy", np.array(E_history))
    np.save(f"loss_history_n{n_x}_{n_y}_2d_adaptativo.npy", np.array(loss_history))
    np.save(f"tiempo_entrenamiento_n{n_x}_{n_y}_2d_adaptativo.npy", np.array([end - start]))


if __name__ == "__main__":
    n_x = int(input("Introduce el número cuántico n_x (0,1,2,...): "))
    n_y = int(input("Introduce el número cuántico n_y (0,1,2,...): "))
    train_adaptativo_2d(n_x, n_y)
